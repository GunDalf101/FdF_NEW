...
int	colorseve(int keycode, t_fdf *mats)
{
	...
	if (keycode == 18)
		...
		mats->drawer.x0, mats->origin->data[1][0] - mats->drawer.y0, 0xfffafa);
	else if (keycode == 19)
		red_green(mats);
	else if (keycode == 20)
		earth(mats);
		...
		my_mlx_pixel_put(&mats->img, mats->origin->data[0][0] + \
		mats->drawer.x0, mats->origin->data[1][0] - mats->drawer.y0, 0x000000);
	else if (keycode == 23)
		castelvania(mats);
	return (keycode);
}
...
int	main(int ac, char **av)
{
	...
	if (ac == 2)
	{
		...
		mats.img.addr = mlx_get_data_addr(mats.img.img,
				&mats.img.bits_per_pixel,
